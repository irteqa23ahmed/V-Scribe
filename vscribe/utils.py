from vscribe import app, questions_collection, db
...
import re,time,os,sys,copy, datetime
...
NAVIGATE_TO_QUESTION = "Alright, going to "+READ_QUESTION
...
def go_to_question_num(q_no,entities):
    try:
        ...
        if (Element.query.get(int(question_no)).question_type == 'MCQ'): read_options(q_no) # Put this in a new intent
        return question_element
    except Exception as e:
        print(e)
        scribe_speaks(UNAVAILABLE_Q)

def read_options(q_no):
    try:
...
def terminate_exam_session(q_no,entities):
    ...
    for x in Element.query.all():
        q_no = x.question_no
        question = x.question
        solution = x.subjective_answer if x.question_type != "MCQ"  else get_selected_option(x.mcq_options)
        pdf.multi_cell(200, 10, txt = SINGLE_RESPONSE_FORMAT.format(q_no,question,solution),  align = 'L') 
        L.append(SINGLE_RESPONSE_FORMAT.format(q_no,question,solution))
    file1.writelines(L)
    file1.close()
    pdf.output("Question.pdf") 
    try:
        db.session.delete(Paper.query.get(1))
        db.session.commit()
    except Exception as e:
        print(e)
